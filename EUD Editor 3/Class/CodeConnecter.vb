...
Public Class CodeConnectGroup
    ...
    End Property
    ' 代码依赖关系
    ' 单位(Units) - 单位_附加单位12, 单位_感染单位
    ' 武器(Weapons) - 单位_对空对地武器, 命令_指定目标时
    ' 运转(Flingy) - 单位_运动参数, 武器_实体图
    ' 精灵(Sprites) - 运转_精灵
    ' 实体图(Images) - 单位_生产模样, 精灵_实体图
    ' 升级(Upgrades) - 单位_护甲, 武器_升级
    ' 技能(Tech) - 武器_不使用, 命令_能量
    ' 命令(Order) - 单位_人类.电脑基本.恢复原状.单位攻击.攻击移动, 命令_未知命令
    ' 文本(Text) - 单位_名称,阶级, 武器_名称, 错误消息, 升级_名称, 技能_名称, 命令_名称

    ' 例如，如果更改升级，则相应的护甲和武器升级的ValueText必须更改。
    ' 例如，如果更改unit_product模样，则必须向更改前的实体图发送Refresh命令，更改后也是如此。
